...
enum CameraType: Int {
  ...
  case cameraCapture = 1
  /// Camera mode for barcode scan
  case cameraBarcodeScan = 2
}

enum CameraRatio: Int {
  /// Ratio 3:4
  case ratio3X4 = 0
  /// Ratio 1:1
  case ratio1X1 = 1
...
private class CameraApiInterfacePigeonCodecReader: FlutterStandardReader {
  override func readValue(ofType type: UInt8) -> Any? {
    ...
    case 131:
      let enumResultAsInt: Int? = nilOrValue(self.readValue() as! Int?)
      if let enumResultAsInt = enumResultAsInt {
        return CameraRatio(rawValue: enumResultAsInt)
      }
      return nil
    default:
...
class QRCodeListener: QRCodeListenerProtocol {
  ...
  func onQrCodeAvailable(qrCode qrCodeArg: String?, completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.native_camera_controller_platform_interface.QRCodeListener.onQrCodeAvailable\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
